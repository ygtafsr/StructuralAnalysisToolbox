...
class Solution:
    ...
    def get_force_history(self, nodes : str | int, direction : Literal["X", "Y", "Z"]) -> list:
...
class Model:
    ...
    def add_load_step(self, analysis : Literal["STATIC", "BUCKLE", "MODAL", "HARMIC" ,"TRANS", "SUBSTR"] = "STATIC",
                      status : Literal["NEW", "RESTART"] = "NEW",
                      step : int = 0, substep : int = 0) -> LoadStep:
        
        ls = LoadStep(step_number=len(self._solution.load_steps)+1,
                      end_time=len(self._solution.load_steps)+1,
                      analysis=analysis, status=status, step=step, substep=substep)
        
        self._solution.load_steps.append(ls)        
    ...
    def solve(self):
        self._add_command("_solve", self._solution)
        self._add_command("_exit_mapdl", None)
