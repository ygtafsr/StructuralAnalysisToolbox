...
if TYPE_CHECKING:
    ...
    from structuralanalysistoolbox.model import Nset, Elset, Surface
...
def _create_element_type(mapdl : core.Mapdl, *args):
    ...
    for keyopt in etype_obj.get_keyopts():
...
def set_force(mapdl: core.Mapdl, force : Force):
    if force.applied_by == "NODE SET":
        mapdl.nsel(type_='S', vmin=force.set.items) # type: ignore
        ...
        _sfcontrol(mapdl=mapdl, direction=force.direction, coordinate_system_id=force.local_cs_id, loaded_area="DEFORMED")
...
def _solve(mapdl: core.Mapdl, *args):  
    ...
    mapdl.allsel()
    mapdl.solve()
    #mapdl.cdwrite(option="ALL", fname="model.cdb")
    mapdl.save()
